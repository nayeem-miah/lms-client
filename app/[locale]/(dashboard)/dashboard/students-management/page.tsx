...
import { Users, Mail, MessageSquare, Search, BookOpen, Calendar, MoreVertical, ExternalLink } from 'lucide-react'
import { useGetInstructorEnrollmentsQuery } from '@/lib/redux/features/enrollments/enrollmentsApi'
...
export default function StudentsManagementPage() {
    ...
    const { data: enrollmentData, isLoading, error } = useGetInstructorEnrollmentsQuery({
        page,
        limit: 10,
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
                    <h1 className="text-3xl font-bold text-slate-100 mb-2">Student Enrollment History</h1>
                    <p className="text-slate-400">Track and manage students enrolled in your courses.</p>
                </motion.div>
                ...
                <div className="relative w-full sm:w-64">
                    ...
                    <input
                        ...
                        className="w-full bg-slate-800 border border-slate-700 rounded-lg pl-10 pr-4 py-2 text-sm text-slate-300 focus:outline-none focus:border-cyan-500"
                    />
                </div>
            </div>
            ...
            {isLoading ? (
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden text-center p-12">
                    ...
                    <p className="text-slate-500 max-w-sm mx-auto">
                        {searchQuery ? "No results match your search." : "When students enroll in your courses, they will appear here."}
                    </p>
                </div>
            ) : (
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-slate-900/50 border-b border-slate-700">
                                ...
                                <th className="px-6 py-4 text-sm font-semibold text-slate-400">Enrolled Date</th>
                                    ...
                                    <tr key={enrollment._id} className="hover:bg-slate-700/20 transition-colors">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                <div className="h-10 w-10 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-full flex items-center justify-center text-xs font-bold text-white uppercase overflow-hidden">
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                <button className="p-2 text-slate-400 hover:text-cyan-400 hover:bg-slate-700/50 rounded-lg transition-all" title="Message Student">
                                                    <MessageSquare className="h-4 w-4" />
                                                </button>
                                                <Link href={`/dashboard/courses/${course?._id}`}>
                                                    <button className="p-2 text-slate-400 hover:text-blue-400 hover:bg-slate-700/50 rounded-lg transition-all" title="View Course">
