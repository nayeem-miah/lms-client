...
export const RegisterPage = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res?.success) {
                toast.success(res.message || t('successToast'), {
                    duration: 4000,
                    icon: '🎉',
                })
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-950 via-purple-950 to-slate-950 px-4 py-12 sm:px-6 lg:px-8 relative overflow-hidden">
            ...
            <Toaster
                position="top-center"
                toastOptions={{
                    style: { background: '#1e293b', color: '#fff', borderRadius: '12px', padding: '16px' },
            ...
            <div className="w-full max-w-md space-y-8 relative z-10">
                ...
                <div className="relative animate-in fade-in slide-in-from-bottom-4 duration-1000" style={{ animationDelay: '200ms' }}>
                    <div className="absolute -inset-0.5 bg-gradient-to-r from-purple-600 via-pink-600 to-cyan-600 rounded-2xl blur opacity-75 animate-pulse" />
                    <div className="relative bg-slate-900/90 backdrop-blur-xl rounded-2xl shadow-2xl border border-slate-800 overflow-hidden">
                        ...
                        <div className="p-8">
                            {error && (
                                <div className="mb-6 bg-red-500/10 border border-red-500/50 text-red-400 px-4 py-3 rounded-lg backdrop-blur-sm animate-in fade-in slide-in-from-top-2 duration-300">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-5">
                                        ...
                                        <div className="mt-3 space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className={`text-xs font-medium ${password.length >= 12 && /[A-Z]/.test(password) && /[a-z]/.test(password) && /[0-9]/.test(password) && /[^A-Za-z0-9]/.test(password)
                                                    ? 'text-green-400' : password.length >= 8 && /[A-Z]/.test(password) && /[a-z]/.test(password) && /[0-9]/.test(password)
                                                        ? 'text-yellow-400' : password.length >= 6
                                                    ...
                                                    {password.length >= 12 && /[A-Z]/.test(password) && /[a-z]/.test(password) && /[0-9]/.test(password) && /[^A-Za-z0-9]/.test(password)
                                                        ? t('passwordStrength.strong') : password.length >= 8 && /[A-Z]/.test(password) && /[a-z]/.test(password) && /[0-9]/.test(password)
                                                            ? t('passwordStrength.good') : password.length >= 6
                                                                ? t('passwordStrength.fair') : t('passwordStrength.weak')
                                                    }
                                                </span>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-2 gap-2 text-xs">
                                                {[
                                                    { condition: password.length >= 8, label: t('passwordStrength.chars') },
                                                    { condition: /[A-Z]/.test(password), label: t('passwordStrength.upper') },
                                                    { condition: /[a-z]/.test(password), label: t('passwordStrength.lower') },
                                                    { condition: /[0-9]/.test(password), label: t('passwordStrength.number') }
                                                ].map((req, i) => (
                                                    <div key={i} className={`flex items-center gap-1.5 ${req.condition ? 'text-green-400' : 'text-slate-500'}`}>
                                                        {req.condition ? <CheckCircle2 className="h-3.5 w-3.5" /> : <XCircle className="h-3.5 w-3.5" />}
                                                        <span>{req.label}</span>
                                                    </div>
                                                ))}
                                            </div>
                                ...
                                <div className="flex items-start space-x-3 pt-2">
                                    <input
                                        id="terms"
                                        type="checkbox"
                                        className="mt-1 h-4 w-4 rounded border-slate-600 bg-slate-800/50 text-purple-600 focus:ring-2 focus:ring-purple-500 transition-all duration-200"
                                        required
                                    />
                                    <label htmlFor="terms" className="text-sm text-slate-400 leading-relaxed">
                                        {t('agreeTo')}{' '}
                                        <Link href="/terms" className="text-purple-400 hover:text-purple-300 transition-colors duration-200">{t('terms')}</Link>
                                        {' '}{t('and')}{' '}
                                        <Link href="/privacy" className="text-purple-400 hover:text-purple-300 transition-colors duration-200">{t('privacy')}</Link>
                                    </label>
                                </div>
                            ...
                            <div className="mt-8">
                                <div className="relative">
                                    ...
                                    <div className="relative flex justify-center text-sm"><span className="bg-slate-900 px-4 text-slate-400">{t('orContinueWith')}</span></div>
                                </div>
                                <div className="mt-6">
                                    <button
                        ...
                        <div className="border-t border-slate-800 bg-slate-900/50 backdrop-blur-sm py-4 px-8">
                            <p className="text-sm text-slate-400 text-center">
                                ...
                                <Link href="/login" className="font-medium text-purple-400 hover:text-purple-300 transition-colors duration-200">{t('signInLink')}</Link>
                            </p>
                        </div>
                    </div>
                </div>
